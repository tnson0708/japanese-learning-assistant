...
export default function Home() {
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-10 px-4 py-10 sm:px-6 sm:py-16 lg:gap-14 lg:py-20">
      ...
      <div className="grid gap-4 sm:grid-cols-3 lg:gap-6">
        {SECTIONS.map((s) => (
          <Link key={s.href} href={s.href}>
            <Card className="h-full transition-colors hover:border-primary/50 hover:bg-accent/40 lg:p-2">
              <CardHeader>
                <span className="text-2xl">{s.kicker}</span>
                <CardTitle className="mt-2 lg:text-xl">{s.title}</CardTitle>
                <CardDescription className="lg:text-base">{s.description}</CardDescription>
              </CardHeader>
            </Card>
          </Link>
        ))}
